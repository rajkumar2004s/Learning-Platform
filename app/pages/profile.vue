<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 md:pt-0 pt-24">
    <!-- Header Section -->
    <div class="bg-gradient-to-r from-blue-600 to-[#080b0f] shadow-lg w-[100vw] md:w-[82vw]">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-4xl md:text-5xl font-bold text-white mb-2">My Profile</h1>
            <p class="text-indigo-100 text-lg">Manage your account and view your learning progress</p>
          </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 -mt-4">
      <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-[#080b0f] to-blue-300 p-8 text-white">
          <div class="flex flex-col md:flex-row items-center gap-8">
            ...
            <div class="relative group">
              <div class="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-white shadow-lg">
                <img
                  :src="previewImage || editedPhotoURL || user.photoURL || defaultAvatar"
                  alt="Profile Picture"
            ...
            <div class="flex-1 text-center md:text-left">
              ...
              <p v-else class="text-xl text-indigo-100 mb-2">{{ user.email }}</p>
              <div v-if="editMode" class="mb-4">
                <select
                  v-model="editedRole"
              ...
              <div v-else class="flex items-center justify-center md:justify-start gap-2">
            ...
            <div class="flex flex-col gap-3">
              ...
              <div v-if="editMode" class="flex gap-3">
                ...
                >
                  <svg v-if="saveLoading" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          <!-- Status Messages -->
          <div v-if="saveError || saveSuccess" class="mt-6 p-4 rounded-lg" :class="saveError ? 'bg-red-500/20 border border-red-400' : 'bg-green-500/20 border border-green-400'">
            <p :class="saveError ? 'text-red-100' : 'text-green-100'" class="text-center font-medium">
        ...
        <div class="p-8">
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
              <div class="bg-gradient-to-br from-slate-50 to-blue-50 rounded-xl p-6 shadow-sm border border-slate-200">
                ...
                <div class="space-y-4">
                  <div class="flex items-center p-3 bg-white rounded-lg border border-slate-200">
                    ...
                    <div>
                      ...
                      <p class="font-medium text-slate-800">{{ user.name || user.displayName || 'User' }}</p>
                    </div>
                  </div>
                  <div class="flex items-center p-3 bg-white rounded-lg border border-slate-200">
                    <svg class="w-5 h-5 text-indigo-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div class="flex items-center p-3 bg-white rounded-lg border border-slate-200">
                    ...
                    <div>
                      ...
                      <p class="font-medium text-slate-800">{{ user.role || 'User' }}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div class="space-y-4">
                  <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-4 bg-white rounded-lg border border-slate-200">
                      <p class="text-2xl font-bold text-indigo-600">{{ enrolledCourses.length }}</p>
                  ...
                  <div class="p-4 bg-white rounded-lg border border-slate-200">
                    <p class="text-lg font-semibold text-slate-800">Total Spent</p>
                    <p class="text-2xl font-bold text-indigo-600">${{ totalSpent.toFixed(2) }}</p>
                  </div>
                  <div class="p-4 bg-white rounded-lg border border-slate-200">
                    <p class="text-lg font-semibold text-slate-800 mb-3">Completion Rate</p>
                    <div class="flex justify-center">
                      <CircularProgressBar :percentage="Number(completedPercentage)" size="80" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="lg:col-span-1">
              <div class="bg-gradient-to-br from-slate-50 to-blue-50 rounded-xl p-6 shadow-sm border border-slate-200">
                ...
                <div v-if="earnedBadges.length > 0" class="grid grid-cols-2 gap-4">
                  <div
                  ...
                  >
                    ...
                    <h4 class="font-semibold text-slate-800 text-sm">{{ badge.name }}</h4>
                ...
                <div v-else class="text-center py-8">
                  ...
                  <p class="text-sm text-slate-400 mt-1">Complete courses to earn badges!</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="bg-gradient-to-br from-slate-50 to-blue-50 rounded-xl p-6 shadow-sm border border-slate-200">
              ...
              <div v-if="enrolledCourses.length > 0" class="grid gap-4">
                ...
                >
                  <div class="flex items-center justify-between">
                    <div class="flex-1">
                      <h4 class="font-semibold text-slate-800 group-hover:text-indigo-600 transition-colors">
                        {{ course.title }}
                      </h4>
                      <div class="flex items-center mt-2">
                        ...
                        <span class="text-sm text-slate-600">{{ course.progress ?? 0 }}%</span>
                      </div>
                    </div>
                    <div class="ml-4">
                      <CircularProgressBar :percentage="Number(course.progress ?? 0)" size="50" color="#6366f1" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div v-else class="text-center py-12">
                ...
                <h4 class="text-lg font-medium text-slate-600 mb-2">No enrolled courses</h4>
                <p class="text-slate-500">Start your learning journey by enrolling in a course!</p>
              </div>
            </div>
          </div>
        </div>
...
const completedPercentage = computed(() => {
  ...
  return Math.round((completedCourses.value / enrolledCourses.value.length) * 100);
});

const badges = ref([
  { id: 1, name: "Beginner", description: "Completed 1 course", icon: "🎓" },
  { id: 2, name: "Intermediate", description: "Completed 5 courses", icon: "🏅" },
